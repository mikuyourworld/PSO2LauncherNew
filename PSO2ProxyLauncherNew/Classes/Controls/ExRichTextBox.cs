...
namespace PSO2ProxyLauncherNew.Classes.Controls
{
    ...
    public class ExRichTextBox : System.Windows.Forms.RichTextBox
    {
        ...
        private string GetDocumentArea(string _text, Font _font)
        {
            ...
            // Font.SizeInPoints
            _doc.Append("\\fs");
            _doc.Append(Convert.ToInt32(Math.Truncate(Math.Round((2 * _font.SizeInPoints)))));
            ...
            _doc.Append(_text.Replace(Constants.vbLf, "\\par "));
        ...
        private string GetImagePrefix(Image _image)
        {
            ...
            int picw = Convert.ToInt32(Math.Truncate(Math.Round((_image.Width / xDpi) * HMM_PER_INCH)));

            // Calculate the current height of the image in (0.01)mm
            int pich = Convert.ToInt32(Math.Truncate(Math.Round((_image.Height / yDpi) * HMM_PER_INCH)));

            // Calculate the target width of the image in twips
            int picwgoal = Convert.ToInt32(Math.Truncate(Math.Round((_image.Width / xDpi) * TWIPS_PER_INCH)));

            // Calculate the target height of the image in twips
            int pichgoal = Convert.ToInt32(Math.Truncate(Math.Round((_image.Height / yDpi) * TWIPS_PER_INCH)));
        ...
        extern static uint GdipEmfToWmfBits(IntPtr _hEmf, uint _bufferSize, byte[] _buffer, int _mappingMode, EmfToWmfBitsFlags _flags);
        ...
        private string GetRtfImage(Image _image)
        {
            ...
            try
            {
                ...
                using (Graphics v_graphics = this.CreateGraphics())
                {

                    // Get the device context from the graphics context
                    _hdc = v_graphics.GetHdc();

                    // Create a new Enhanced Metafile from the device context
                    _metaFile = new Metafile(_stream, _hdc);

                    // Release the device context
                    v_graphics.ReleaseHdc(_hdc);
                }

                // Get a graphics context from the Enhanced Metafile
                using (Graphics v_graphics = Graphics.FromImage(_metaFile))
                {

                    // Draw the image on the Enhanced Metafile

                    v_graphics.DrawImage(_image, new Rectangle(0, 0, _image.Width, _image.Height));
                }
        ...
        {
            return _originalRtf.Replace(Constants.vbNullChar, "");
        }
        ...
        private void OriAppendText(object obj)
        {
            ...
            {
                RtfColor myColor = this.TextColor;
                if (theQueueNode.TextColor == this.TextColor)
                    this.AppendTextAsRtf("\n ", this.Font, this.TextColor);
                else
        ...
        {
            this.AppendText(msg, this.TextColor);
        }

        public void AppendText(string msg, RtfColor textColor)
        {
            this.myQueue.Enqueue(new AppendTextInfo(msg, textColor));
            this.StartQueue();
        }

        internal class AppendTextInfo
        {
            ...
            public AppendTextInfo(string msg) : this(msg, RtfColor.Black)
            { }

            public AppendTextInfo(string msg, RtfColor theColor)
            {
                this.Message = msg;
