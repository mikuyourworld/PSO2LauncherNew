...
namespace Leayal.Forms
{
    ...
    public class ColorSlider : Control
    {
        ...
        private DerpedPopupForm valuePopup;

        private Color _valueTextColor;
        public Color ValueTextColor
        {
            get { return this._valueTextColor; }
            set
            {
                this._valueTextColor = value;
                if (valuePopup != null)
                {
                    valuePopup.ForeColor = _valueTextColor;
                    valuePopup.Invalidate();
                }
            }
        }

        private Color _valueTextBackColor;
        public Color ValueTextBackColor
        {
            get { return this._valueTextBackColor; }
            set
            {
                this._valueTextBackColor = value;
                if (valuePopup != null)
                {
                    valuePopup.BackColor = _valueTextBackColor;
                    valuePopup.Invalidate();
                }
            }
        }
        ...
        /// <summary>
        /// {0} for current value; {1} for minimum; {2} for maximum
        /// </summary>
        public string ValueTextFormat
        {
            get { return this._valueTextFormat; }
            set
            {
                this._valueTextFormat = value;
                if (valuePopup != null)
                    valuePopup.Invalidate(); ;
            }
        }
        ...
        /// <summary>
        /// Fires when Slider position is changing
        /// </summary>
        [Description("Event fires when Slider position is changing")]
        [Category("Action")]
        ...
        private AvailableIntRange _availableRange;
        [Description("Set Slider avaiable value")]
        [Category("ColorSlider")]
        public AvailableIntRange ValueAvailableRange
        {
            get { return this._availableRange; }
            set
            {
                if (value.MaximizeValue > this.Maximum)
                    value.MaximizeValue = this.Maximum;
                if (value.MinimizeValue < this.Minimum)
                    value.MinimizeValue = this.Minimum;
                if (this._availableRange != value)
                {
                    this._availableRange = value;
                    int realmin = Math.Max(barMinimum, this._availableRange.MinimizeValue);
                    int realmax = Math.Min(barMaximum, this._availableRange.MaximizeValue);
                    if (trackerValue < realmin)
                    {
                        trackerValue = realmin;
                    }
                    else if (trackerValue > realmax)
                    {
                        trackerValue = realmax;
                    }
                    this.Invalidate();
                }
            }
        ...
        private int _step;
        /// <summary>
        /// Gets or sets the step value of Slider.
        /// </summary>
        ...
        public int Step
        {
            ...
            set
            {
                if (value < 1) //assume your step is 0, it mean after 100 years or more, you can slide your slider from 0 to 1
                {
                    prelist = null;
                    this._step = 0;
                }
                ...
                {
                    int myNum = barMaximum - barMinimum;
                    if (value > myNum)
                        throw new ArgumentOutOfRangeException("Value is outside appropriate range (min, max)");
                    else
                    {
                        int stepcount = myNum / value;
                        if ((stepcount * value) == myNum)
                        {
                            this._step = value;
                            if (stepcount == 1)
                                prelist = new int[2] { barMinimum, barMaximum };
                            else
                            {
                                prelist = new int[stepcount + 1];
                                for (int i = 0; i <= stepcount; i++)
                                    prelist[i] = (i * value) + barMinimum;
                            }
                            this.adjustValueToNearestStep(this.trackerValue);
                            this.Invalidate();
                        }
                        else
        ...
        private int[] prelist;
        private void adjustValueToNearestStep(int value)
        {
            if (prelist != null && prelist.Length > 0)
            {
                float halfStep = this._step / 2F;
                int currentnum;
                for (int i = 0; i < prelist.Length; i++)
                {
                    currentnum = prelist[i];
                    if (value == currentnum)
                        break;
                    else if (value < (currentnum + halfStep) && value > (currentnum - halfStep))
                    {
                        this.trackerValue = currentnum;
                        break;
                    }
                }
            }
        ...
        public int Minimum
        {
            ...
            set
            {
                ...
                {
                    barMinimum = value;
                    if (this._availableRange.MinimizeValue < value)
                        this._availableRange.MinimizeValue = value;
                    int realmin = Math.Max(barMinimum, this._availableRange.MinimizeValue);
                    if (this._availableRange.MaximizeValue < realmin)
                        this._availableRange.MaximizeValue = realmin;
                    if (trackerValue < realmin)
                    {
                        trackerValue = realmin;
                        this.OnValueChanged(System.EventArgs.Empty);
        ...
        public int Maximum
        {
            ...
            set
            {
                ...
                {
                    barMaximum = value;
                    if (this._availableRange.MaximizeValue > value)
                        this._availableRange.MaximizeValue = value;
                    int realmax = Math.Min(barMaximum, this._availableRange.MaximizeValue);
                    if (trackerValue > realmax)
                    {
                        trackerValue = realmax;
                        this.OnValueChanged(System.EventArgs.Empty);
                    }
        ...
        public ColorSlider(int min, int max, int value) : base()
        {
            ...
            Minimum = min;
            Maximum = max;
            _availableRange.MinimizeValue = min;
            _availableRange.MaximizeValue = max;
            Value = value;
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (Capture & e.Button == MouseButtons.Left)
            {
                ...
                if (prelist != null && prelist.Length > 0)
                    this.adjustValueToNearestStep((int)(p * coef + barMinimum));
                else
        ...
        private void SetProperValue(int val)
        {
            int realmin = Math.Max(barMinimum, this._availableRange.MinimizeValue);
            int realmax = Math.Min(barMaximum, this._availableRange.MaximizeValue);
            if (val < realmin) Value = realmin;
            else if (val > realmax) Value = realmax;
            else Value = val;
        }
        ...
        private void ShowPopup(int Value)
        {
            ...
            {
                this.valuePopup.Close();
            }
            //if (string.IsNullOrWhiteSpace(this.ValueTextFormat)) return;
            
            if (!thumbRect.IsEmpty)
            {
                this.valuePopup = SetupAnotherPopup();
                this.valuePopup.Show(); //FormNotShown
                this.EditPopup(Value);
            }
        }

        private void EditPopup(int Value)
        {
            //if (string.IsNullOrWhiteSpace(this.ValueTextFormat)) return;
            if (!thumbRect.IsEmpty && this.valuePopup != null)
            {
                this.valuePopup.ExTooltipText = string.Format(this.ValueTextFormat, this.trackerValue, this.barMinimum, this.barMaximum);
                Size textSize = TextRenderer.MeasureText(this.valuePopup.ExTooltipText, this.valuePopup.Font);
                this.valuePopup.Size = new Size(textSize.Width + 2, textSize.Height + 2);
                ...
                int asdasd = (((trackerValue - barMinimum) * (ClientRectangle.Width - thumbSize)) / (barMaximum - barMinimum)) + (thumbSize / 2) - (this.valuePopup.Size.Width / 2);
                this.valuePopup.DesktopLocation = this.PointToScreen(new Point(asdasd, -1 * this.valuePopup.Size.Height));
                this.valuePopup.Invalidate();
            }
        }

        private void ValuePopup_Paint(object sender, PaintEventArgs e)
        {
            DerpedPopupForm fffff = sender as DerpedPopupForm;
            if (fffff != null)
            {
                Rectangle newRect = new Rectangle(new Point(1, 1), new Size(fffff.Width - 2, fffff.Height - 2));
                TextRenderer.DrawText(e.Graphics, fffff.ExTooltipText, fffff.Font, newRect, fffff.ForeColor, fffff.BackColor);
            }
        }
        ...
        private DerpedPopupForm SetupAnotherPopup()
        {
            var result = new DerpedPopupForm(true);
            result.Opacity = 0.8D;
            result.Paint += ValuePopup_Paint;
            //result.PaintBackground += CurrentTooltip_PaintBackground;
            result.FormBorderStyle = FormBorderStyle.None;
            result.BackColor = this.ValueTextBackColor;
            //result.BackColor = th;
            result.Font = this.Font;
            result.StartPosition = FormStartPosition.Manual;
            result.ForeColor = this.ValueTextColor;
            return result;
        }

        private class DerpedPopupForm : System.Windows.Forms.Form
        {
            int nopeCount;
            bool _useFading;
            ...
            public DerpedPopupForm(bool useFading) : base()
            {
